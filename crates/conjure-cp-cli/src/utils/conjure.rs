...
use conjure_cp::solver::adaptors::{Minion, Smt};
...
pub fn get_solutions(
    ...
    if let Some(Expression::DominanceRelation(_, dom_rel)) = model.dominance.clone() {
        ...
        match solver {
            ...
            SolverFamily::Smt => {
                get_solutions_no_dominance(Smt::default(), model, num_sols, solver_input_file)
            }
        }
    }
}
...
pub fn get_solutions_with_dominance(
    ...
    loop {
        ...
        let solutions = match solver {
            ...
            SolverFamily::Smt => {
                get_solutions_no_dominance(Smt::default(), model.clone(), 1, solver_input_file)?
            }
        };
    ...
    for sol in results.iter() {
        ...
        let sols = match solver {
            ...
            SolverFamily::Smt => {
                get_solutions_no_dominance(Smt::default(), model_copy, -1, solver_input_file)?
            }
        };
...
pub fn sub_in_solution_into_dominance_expr(
    ...
    match expr {
        FromSolution(_, name) => {
            if let Atom::Reference(ptr)= &**name {
